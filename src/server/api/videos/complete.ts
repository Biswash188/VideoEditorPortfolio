...
import { completeUploadSession, createVideo, getUploadSession, getVideoByGoogleDriveFileId, updateVideo } from "../../database/videos.js";
import { ApiError, withErrorHandling } from "../../errors.js";
import { drivePlaybackUrl, getDriveFile, makeDriveFilePublic } from "../../google-drive.js";
...
export default { fetch: withErrorHandling(async (request) => {
  ...
  if (existing) {
    await makeDriveFilePublic(input.googleDriveFileId);
    await updateVideo(existing.id, { isPublished: metadata.isPublished });
    await completeUploadSession(session.id, input.googleDriveFileId);
    return Response.json({ completed: true, videoId: existing.id });
  }
  const file = await getDriveFile(input.googleDriveFileId);
  if (file.trashed || file.mimeType !== session.mimeType || Number(file.size) !== session.fileSize || !file.parents?.includes(process.env.GOOGLE_DRIVE_FOLDER_ID ?? "")) throw new ApiError(400, "UPLOAD_MISMATCH", "The Google Drive file does not match this upload session.");
  ...
  return Response.json({ completed: true, videoId: video.id }, { status: 201 });
}) };
